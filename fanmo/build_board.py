...
// 시즌별+통산 누적 기록을 보여주는 player.html을 연다 — KBO 공식 기록실처럼 선수
// 하나를 따로 떼어서 보는 화면. player_code가 없는 행(코스트 CSV 미등록 등)은 열
// 대상이 없으니 조용히 무시한다. 투수 행에만 있는 role 필드로 타자/투수를 가린다.
// 실제로 어디에 여는지(현재 탭 대체/새 탭/새 창)는 옵션 페이지(options.html)에서
// 고른 값을 openPlayerLink()가 공통 처리한다 — 이 함수는 URL만 만든다.
function openPlayerPage(row) {
  if (!row.player_code) return;
  const role = row.role ? 'pitchers' : 'batters';
  openPlayerLink(row.player_code, role, row.name || '');
}

// 옵션 페이지에서 고른 "선수 기록 열기 방식"(playerLinkMode: replace/tab/window, 기본값
// window)을 따라 player.html을 연다 — lp_board.html·records.html이 공통으로 쓴다.
